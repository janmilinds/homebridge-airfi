...
export class AirfiHomebridgePlatform implements DynamicPlatformPlugin {
  ...
  /**
   * Add an existing accessory to the platform.
   *
   * @param accessory The accessory to add.
   * @param device The device configuration.
   * @param displayName The display name for the accessory.
   */
  private addExistingAccessory(
    accessory: PlatformAccessory<AirfiDeviceContext>,
    device: AirfiDeviceConfig,
    displayName: string
  ) {
    ...
    airHandlingUnitAccessory
      .once('initialized', () => {
        this.log.info('Loading accessory from cache:', accessory.displayName);
      })
      .once('error', () => {
        this.log.warn(
          'Failed to load accessory from cache:',
  ...
  /**
   * Add a new accessory to the platform.
   *
   * @param uuid The UUID for the accessory.
   * @param device The device configuration.
   * @param displayName The display name for the accessory.
   */
  ...
  private addAccessories() {
    for (const device of this.config.devices) {
      ...
      if (existingAccessory) {
        this.addExistingAccessory(existingAccessory, device, displayName);
      } else {
        this.addNewAccessory(uuid, device, displayName);
      }
    }
  }
